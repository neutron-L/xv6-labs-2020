...
int vma_handler(uint64 cause, uint64 addr)
{
    ...
    if (v == &p->mmap)
    {
        printf("vma_handler: vma does not exist\n");
        return -1;
    }

    // Step 2. check permissions
    if ((cause == 0xd && !(v->prot & PROT_READ)) || (cause == 0xf && !(v->prot & PROT_WRITE)))
    {
        printf("vma_handler: permission deny\n");
        return -1;
    }

    // Step 3. allocate page
    char *pa;
    if ((pa = kalloc()) == 0)
    {
        printf("vma_handler: kalloc failed\n");
        return -1;
    }
    ...
    {
        kfree(pa);
        printf("vma_handler: mmappages failed\n");
        return -1;
    }

    // Step 4. read file to init page
    ilock(v->fp->ip);
    uint off = v->offset + (addr - v->addr);
    int tot = v->fp->ip->size - off;
    if (tot > PGSIZE)
        tot = PGSIZE;
    int r = 0;
    int origin_tot = tot;
    while (tot && (r = readi(v->fp->ip, 0, (uint64)pa, off, tot)) > 0)
    {
        off += r;
        tot -= r;
    }
